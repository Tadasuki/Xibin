...
async function refreshData(reason, options = {}) {
  try {
    ...
    console.error(error);
    el.focusPanel.innerHTML = '<div class="empty-state">读取 <code>data/恋斌场.json</code> 失败，请通过开发服务器访问页面。</div>';
  }
}

function applyInitialEntrySelection() {
  if (state.initialEntryApplied || !Array.isArray(state.data?.stories)) {
    return;
  }
  ...
  if (Number.isInteger(entry)) {
    state.selectedIndex = clamp(entry, 0, Math.max(0, state.data.stories.length - 1));
  }
}
